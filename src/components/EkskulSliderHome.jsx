...
const EkskulSlider = ({ onLoadFinish }) => {
  ...
  useEffect(() => {
    const fetchClubs = async () => {
      try {
        ...
        if (onLoadFinish) onLoadFinish(); // penting buat matikan loading
      }
    };
  ...
  return (
    <div className="bg-gradient-to-b from-white via-blue-200 to-blue-100 py-10 px-4 sm:px-8 md:px-12 lg:px-16 xl:px-20">
            ...
            <SwiperSlide key={club.hash_id} className="flex justify-center px-14 sm:px-0">
              {({ isActive }) => (
                <motion.div
                  ...
                  className="bg-gradient-to-br from-white via-blue-50 to-blue-100 border-2 border-blue-500 shadow-[0_4px_20px_rgba(0,0,0,0.4)] rounded-2xl p-6 text-center w-full max-w-xs"
                >
                  ...
                  {/* <button className="mt-2 px-4 py-1 bg-blue-100 border border-blue-400 text-blue-700 rounded-full text-sm hover:bg-blue-200 transition">
                    Lihat
                  </button> */}
                </motion.div>
              )}
